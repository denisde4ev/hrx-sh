...
case $1 in --help)
	printf %s\\n \
		"Usage: ${0##*/} [-c|-x] [-f OUTFILE] [--boundary BND] [--] [FILES...]" \
		"" \
...
boundary='<===>' # default
case ${opt_boundary_val+x} in x)
	case $opt_boundary_val in # first check for somethnig that looks canda valid; and then check inside if invalid
		[1-9]*)
			case $opt_boundary_val in *[!0-9]*) die "E: invalid --boundary value, invalid number";; esac
			while case $opt_boundary_val in [1-9]*) ;; *) false; esac
				boundary="${boundary}="
				opt_boundary_val=$(( opt_boundary_val - 1 ))
			done
			boundary="${boundary}>"
		;;
		=*)
			case $opt_boundary_val in *[!=]*) die "E: invalid --boundary value, invalid char in =*";; esac
			boundary="<${boundary}>"
		;;
		\<=*\>)
			case $opt_boundary_val in \<*[!=]*\>) die "E: invalid --boundary value, invalid char in <=*>";; esac
